...
class UserDataService {
    constructor() {
        console.log('UserDataService constructor called');
        ...
        console.log('UserDataService constructor complete');
    }
    ...
    loadUserData() {
        try {
            ...
            const savedData = storageService.getItem(this.STORAGE_KEY);

            if (savedData) {
                console.log('Found saved user data, parsing...');
                this.userProfile = savedData;

                // Convert date strings back to Date objects if they exist
                if (this.userProfile.joinDate) {
                    this.userProfile.joinDate = new Date(this.userProfile.joinDate);
                }
                if (this.userProfile.dailyStats) {
                    this.userProfile.dailyStats = this.userProfile.dailyStats.map(stat => ({
                        ...stat,
                        date: new Date(stat.date)
                    }));
                }
                if (this.userProfile.sessionNotes) {
    ...
    saveUserData() {
        try {
            ...
            const storageService = StorageService.getInstance();
            storageService.setItem(this.STORAGE_KEY, this.userProfile);
